...
public class MainReport {
	...
	public static void screenShotHyperLink(String testName, int count) {
		try {
			...
			int counter1 = MainReport.imageCounter - 1;
	...
	public void addToLog() {
		try {
			...
			Double totalTest = MainListener.passedTests.size()
					+ MainListener.failedTests.size()
					+ MainListener.skippedTests.size() + 0.0;
			int passedTests = MainListener.passedTests.size();
			int failedTests = MainListener.failedTests.size();
			int skippedTests = MainListener.skippedTests.size();
	...
	public void mainReport(int count) {
		try {
			...
			bw.write("<header>");
			bw.write("<div id='logo'><img alt='company' height='74' src='"
					+ new File("").getAbsolutePath()
					+ "\\images\\company.png' width='176'></div>");
			bw.write("<div id='rightlogo'><img alt='project' height='99' src='"
					+ new File("").getAbsolutePath()
					+ "\\images\\project.png' width='176'></div>");
			bw.write("</header>");
			bw.write("</div>");
			bw.write("</div>");
			// ----------------Change START---------------------------
	...
	public boolean createlogfile(String Filename, String className, int count) {
		...
		try {
			...
			writer.write("<header>");
			writer.write("<div id='logo'><img alt='company' height='74' src='"
					+ new File("").getAbsolutePath()
					+ "\\images\\company.png' width='176'></div>");
			writer.write("<div id='rightlogo'><img alt='project' height='99' src='"
					+ new File("").getAbsolutePath()
					+ "\\images\\project.png' width='176'></div>");
			writer.write("</header>");
			writer.write("</div>");
	...
	public void reporter(List<String> passedTests, List<String> failedTests,
		...
		writer.write("<div id='logo'><img alt='company' height='74' src='"
				+ new File("").getAbsolutePath()
				+ "\\images\\company.png' width='176'></div>");
		writer.write("<div id='rightlogo'><img alt='projects' height='99' src='"
				+ new File("").getAbsolutePath()
				+ "\\images\\project.png' width='176'></div>");
		writer.write("</header>");
		writer.write("</div>");
		...
		if (passedTests.size() > 0) {
			...
			Iterator<Object[]> passedParameters = MainListener.passedParametervalues
					.iterator();
			Iterator<String> passedClasses = MainListener.passedClassNames
					.iterator();
			Iterator<String> passedFileNames = MainListener.passedLogFiles
					.iterator();
		...
		if (failedTests.size() > 0) {
			...
			Iterator<Object[]> failedParameters = MainListener.failedParametervalues
					.iterator();
			Iterator<String> failedClasses = MainListener.failedClassNames
					.iterator();
			Iterator<String> failedFileNames = MainListener.failedLogFiles
					.iterator();
		...
		if (skippedTests.size() > 0) {
			...
			Iterator<Object[]> skippedParameters = MainListener.skippedParametervalues
					.iterator();
			Iterator<String> skippedClasses = MainListener.skippedClassNames
					.iterator();
			while (iter.hasNext()) {
	...
	public void after() throws IOException {
		...
		addToLog();
		reporter(MainListener.passedTests, MainListener.failedTests,
				MainListener.skippedTests, count1);
